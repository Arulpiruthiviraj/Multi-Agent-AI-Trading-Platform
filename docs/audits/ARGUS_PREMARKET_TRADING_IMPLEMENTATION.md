...
**Status: complete, including live premarket order-idea emission (§12) — with one real correctness gap found and fixed by a post-implementation audit (§13), and two known, honestly-documented residual limitations (§13.3).** This is the mission's required final report (§31). It is written to the same evidence standard as the rest of this codebase's audits: every claim below is either a file/line reference, a test-run result, or explicitly marked as not done. Nothing here raises a readiness score — `CLAUDE.md` §0 governs.

Companion documents (Phase 0, produced first, unchanged by this report): `docs/architecture/ARGUS_ARCHITECTURE.md (Premarket / Session-Aware Trading Architecture section)` (current-state audit), `docs/architecture/ARGUS_ARCHITECTURE.md (Premarket / Session-Aware Trading Architecture section)` (gap analysis + architectural decision). Read those first — this report assumes their findings and states what happened *after* them.

---

## 1. One-paragraph answer to "are all phases complete?"

**Yes, as of a second, 2026-09-05 explicit-authorization pass — see §12.** The original pass shipped Phases 0, 1, 3, 4, 5, and 7. A follow-up pass (§11) closed most of the remaining gaps: Phase 2's session-policy-parameterization mechanism now exists (`rankingThresholdsBySession`), Phase 3's `javaQuantScore` is now wired to a real (bounded, once-daily) live caller, Phase 6 gained 5 real end-to-end RTH-handoff integration tests, the extended-hours ADV/liquidity gap (previously `NOT_IMPLEMENTED`) is now closed with a real data source, a real weekend-classification bug in `RegimeEngine.classifyDeskSession()` was fixed, and the mission's UI requirement (§18/§27) shipped as a read-only `PremarketIntelligence.tsx` tab. That pass deliberately left `TradePlanBuilder` shadow-only (§11.6), matching this codebase's own established evidence-before-live-vote precedent (the Java factor-composite activation gating in `docs/architecture/ARGUS_ARCHITECTURE.md (Java Quant Core section)`) — a named, reasoned gate, not an oversight. **The repository owner was told that reasoning explicitly and chose to override it** (§12): `TradePlanBuilder` now emits one real `TRADE_IDEA_GENERATED` per PRIMARY-tier premarket plan, gated behind an explicit `.env` flag this deployment has turned on. Area 2 (rescue-capacity independence-starvation, deferred mid-session in the original pass) was also completed, evidence-grounded — see §9.
...
- `docs/architecture/ARGUS_ARCHITECTURE.md (Premarket / Session-Aware Trading Architecture section)`
- `docs/architecture/ARGUS_ARCHITECTURE.md (Premarket / Session-Aware Trading Architecture section)`
...
## 12. Live `TRADE_IDEA_GENERATED` wiring (2026-09-05, explicit operator authorization — supersedes §11.6)

§11.6 above recorded a deliberate decision NOT to wire `TradePlanBuilder` into live trade-idea emission, and explained the precedent this codebase already has for that decision (the Java factor-composite gate). The user was told that reasoning explicitly, in the same conversation, in response to asking "how do I configure automatic premarket buy/sell" — and, when given a direct choice between (a) leaving it shadow-only, (b) wiring it live now in paper, or (c) building the measurement path first, chose (b) explicitly. This section documents what changed as a result.

### 12.1 What was built

`TradePlanBuilder.ts` gained `emitTradePlanIdea(draft, currentPrice)`: emits exactly one `TRADE_IDEA_GENERATED` per **PRIMARY-tier** TradePlan (never BACKUP/WATCHLIST — a deliberately conservative scope given zero prior track record, chosen unilaterally by the implementer since the user did not specify a tier scope; can be widened later). Gated behind three independent, all-must-pass checks, mirroring `OpportunityScreener.ts`'s own established pattern exactly:

1. `isTradePlanIdeasEnabled()` — new master flag, `ARGUS_TRADE_PLAN_IDEAS_ENABLED` (`config/continuousIntelligence.json`). Off by default in `.env.example`; explicitly turned on in this deployment's `.env`.
2. `isPipelineAgentEnabled('TradePlanBuilder')` — new Mission Control per-agent toggle (`config/pipelineAgents.json`'s `togglableIdeaAgents`), on by default (matching every other togglable agent's `defaultEnabled()` behavior) but independently switchable off from the UI without touching `.env`.
3. `isLiveIdeaGenerationEnabled()` — the same Autobot/session-recovery/campaign-lock composite gate every other entry-idea agent already respects.

Wired into `SnapshotScanner.refreshSnapshotRanks()`'s existing PRE_MARKET branch, immediately after `persistTradePlanDrafts(drafts)` — one call per draft, using the same `inputsBySymbol` snapshot price already in scope (no new network call). A missing/non-positive price fails closed (`INVALID_PRICE`), never fabricates one.

The emitted idea: `agent: 'TradePlanBuilder'`, `side` = the plan's real direction, `confidence` = the plan's real `finalScore` (0–1), `reasoning` = the plan's real thesis text, `strategy: 'PREMARKET_TRADE_PLAN'`. It goes through `eventBus.emitTradeIdea()` — the same `gateTradeIdea()` symbol/price validation, the same `TRADE_IDEA_GENERATED` event every other agent uses. **Nothing about ChiefTrader's consensus math, RiskEngine's 25 gates, or OMS changed.** This is one more independent vote, subject to the same 0.75 consensus bar and 2-independent-agent floor as every other agent — it does not skip debate, does not lower any threshold, and does not touch `CHIEF_APPROVED_IDEA` or `.placeOrder(` directly.

Added a default consensus weight, `TradePlanBuilder: 0.10` (`config/agentWeights.json`), matching `OpportunityScreener`'s own conservative 0.10 default for a similarly new, unproven idea source.

### 12.2 Architecture-protection invariant extended, not bypassed

`src/server/architecture.protection.test.ts` had a real, reviewed, static-scan CI test asserting that **only `OpportunityScreener.ts`** may call `emitTradeIdea()` from anywhere under `src/server/continuous/`. This is exactly the kind of protected invariant `CLAUDE.md` says must not be silently worked around. It was extended deliberately — a new `ALLOWED_EMITTERS_IN_CONTINUOUS` set now also includes `TradePlanBuilder.ts`, with an inline comment dated 2026-09-05, naming the explicit operator authorization as the reason, matching the test's own header convention ("A new entry here is a real architectural decision, not a drive-by import"). The test still fails CI if any *other* file under `src/server/continuous/` calls `emitTradeIdea()` — the invariant is narrowed, not removed.

A second, real consistency check was tripped and fixed honestly rather than routed around: `src/server/core/pipelineAgentRuntime.ts` asserts at module-load time that every catalog entry in `pipelineAgents.json`'s `togglableIdeaAgents` has a registered `{start, stop}` runtime pair. `TradePlanBuilder` has no independent worker of its own (its emission is inline, triggered by `SnapshotScanner`'s own already-running cycle) — added a no-op pair, identical in shape and reasoning to `NewsAgent`'s existing no-op entry, with a comment explaining why.

A third issue, self-inflicted and unrelated to any real invariant: the doc comments this pass added to `TradePlanBuilder.ts` and `architecture.protection.test.ts` originally contained the literal text `` `.placeOrder(` `` as a plain-English negative claim ("never calls placeOrder") — which is exactly the substring `architecture.protection.test.ts`'s own regex scans for. Fixed by rephrasing those comments to avoid the literal substring; the code itself never called `.placeOrder(` at any point.

### 12.3 Extended-hours execution enabled

For a PRIMARY-tier idea emitted PRE_MARKET to result in a genuine premarket order (not merely an idea that waits, unfilled, for RTH), `EXTENDED_HOURS_EXECUTION_ENABLED` (§11 of the original Phase 5 work, built earlier the same day) must also be `true` — this deployment's `.env` was updated to enable it, alongside `ARGUS_TRADE_PLAN_IDEAS_ENABLED`. Gate 25 (`extended_hours_execution_policy`) and its real ADV/spread/quote-age/notional checks (§11.5) apply exactly as before — this change does not weaken, skip, or special-case gate 25 for TradePlan-sourced ideas.

### 12.4 What this does NOT change

- RiskEngine's 25 gates, their order, or their thresholds — unchanged.
- ChiefTrader's consensus math, weights resolution, or debate logic — unchanged. `TradePlanBuilder` is just one more named agent with a config-driven default weight, same as `OpportunityScreener`.
- `TradePlanShadowTracker` (§11.6) — still runs, still records a shadow prediction the first time a plan reaches `VALID` during REGULAR-session revalidation. This is a separate mechanism from the PRE_MARKET-time `emitTradePlanIdea()` call and both now run concurrently.
- BACKUP/WATCHLIST-tier plans — still never emit an idea. Only PRIMARY (top `primaryCount`, default 3, `TradePlanBuilder.DEFAULT_TRADE_PLAN_THRESHOLDS`).
- LIVE readiness — unaffected. `evaluateLiveReadiness()` still reports `LIVE_NO_GO`; this entire mechanism only ever produces PAPER orders (`PAPER_TRADING_ONLY=true` in this deployment, unaffected by anything in this section).

### 12.5 Test evidence

- `TradePlanBuilder.test.ts` gained a new `describe('emitTradePlanIdea ...)` block (7 new tests): flag-off no-op, non-PRIMARY-tier no-op (both BACKUP and WATCHLIST), agent-disabled no-op, Autobot-off no-op, invalid-price no-op (null/zero/negative, each separately), and the real-emission success case (subscribes to `TRADE_IDEA_GENERATED`, asserts exactly one event with the correct symbol/side/agent/currentPrice/traceId).
- `architecture.protection.test.ts`'s extended allowlist test re-verified green with both `OpportunityScreener.ts` and `TradePlanBuilder.ts` as the only two permitted emitters.
- Full suite re-run after this section's changes: **see the summary message delivered alongside this report for the exact final count** (this document does not assert a number in advance of that run completing).

### 12.6 Operator action required to see this take effect

Both new `.env` flags were written to this deployment's `.env` file. Node processes read `.env` only at boot (via dotenv), so **the running Argus instance must be restarted** (`./argus restart`, or stop/start via `argus-cli`, or a manual `Ctrl+C` + `npm run dev`/`dev:server-only` relaunch) before `ARGUS_TRADE_PLAN_IDEAS_ENABLED`/`EXTENDED_HOURS_EXECUTION_ENABLED` take effect. Editing the file alone does not change a live process's behavior. Autobot's own enabled/disabled state persists in SQLite independently of this restart, so — provided Autobot was already on before the restart — no separate manual Autobot re-enable step is needed afterward.

---

## 13. Post-implementation forensic audit (2026-09-05, requested review of §12 before unattended operation)

After §12 shipped, the user requested (via a second-opinion review) a forensic audit of the full new path — `Discovery → Java Quant → Premarket ranking → TradePlan → agent evidence → ChiefTrader → RiskEngine → OMS → broker` — before treating it as production-ready, specifically flagging: (1) whether `TradePlanBuilder` duplicates or conflicts with the Java Quant Core migration, (2) whether PRIMARY plans can actually reach ChiefTrader with fresh prices, and (3) whether the extended-hours gates behave correctly under real paper-market conditions. This section reports what was actually checked, against the real code, not re-asserted from §12.

### 13.1 Java Quant Core conflict check — no conflict found

Traced `emitTradePlanIdea()`'s inputs: `draft.confidence` is `candidate.finalScore`, `draft.direction` is derived from `rawMomentumPct`'s sign — both already computed upstream by `ComposableRanking.runRankingCycle()` before `TradePlanBuilder` ever sees them. `TradePlanBuilder.ts` performs no independent numeric/quant calculation of its own (`buildThesis`/`deriveEntryZone`/`deriveInvalidationLevel` are simple arithmetic on already-fetched bar fields — pre-existing since Phase 4E, unchanged by this pass, not the kind of technical-indicator/strategy-evaluation calculation `CLAUDE.md`'s Java Engine Authority section governs). When `javaQuantScore` is available (§11.4's bounded PRE_MARKET wiring), it is already blended into `finalScore` upstream — `TradePlanBuilder` never talks to `quantCoreBridge` or any Java engine directly. **Conclusion: `TradePlanBuilder` is a consumer of ComposableRanking's already-Java-aware score, not a second, competing quant calculation. No duplication, no split-brain.**

### 13.2 Price-freshness check — a real gap found and fixed

This is the substantive finding. Traced the actual gate that would evaluate a `TradePlanBuilder` idea:

- `RiskEngine.ts` gate 13 (`data_freshness`, line ~557) calls `marketDataWorker.getLatestPriceAgeMs(proposal.symbol)` — this reads `MarketDataWorker`'s own **live WebSocket tick cache** (`latestPriceTimestamps`), a completely different data source than the **REST snapshot** `ComposableRanking`/`SnapshotScanner` score candidates from.
- `evaluateQuoteFreshness()` (`src/server/core/marketDataQuality.ts`) fails closed on a null age: `'UNKNOWN_FRESHNESS: no tick age for this symbol... No new order until a real tick is observed.'` — by design, matches DEF-08.
- `MarketDataWorker` only actively WebSocket-streams up to `maxActiveSubscriptions` (12) symbols at once. `TradePlanBuilder`'s PRIMARY-tier candidates are drawn from `getSnapshotScanUniverse()` (~122 symbols: seed + watch + campaign + momentum universe) — most of that universe is **not** in the active 12-slot pool at any given moment.
- **Before this fix**, `emitTradePlanIdea()` did nothing to get the symbol onto MarketDataWorker's radar. A PRIMARY-tier idea for a symbol outside the active pool would very likely fail gate 13 immediately — not because of any real risk judgment about the trade, but because nothing had ever asked `MarketDataWorker` to track that symbol. Since `TradePlanBuilder` fires **once per trading day** (at PRE_MARKET plan-build time, no retry), a gate-13 rejection on that single attempt meant that symbol's premarket idea was dead for the day.

**Fix applied:** `emitTradePlanIdea()` now calls `marketDataWorker.requestTemporaryDataRescue(draft.symbol, 'premarket_trade_plan_idea', { requestClass: 'EXPLORATION' })` immediately before emitting — the same bounded, already-tested rescue mechanism `OpportunityDiscovery` uses for this identical class of problem (a real, existing tool, not a new mechanism invented for this fix). This gives the symbol a real subscription and a real chance to accumulate a live tick before ChiefTrader's (non-instant, multi-provider) consensus debate concludes.

**Stated honestly, not oversold:** a rescue grants a *subscription*, not an instant tick — this materially improves the odds gate 13 passes but does not guarantee it, especially in premarket hours when tick flow is naturally thinner than RTH. The new `TradePlanIdeaResult.rescueRequested` field reports whether the rescue was actually granted (vs. denied for capacity reasons) so this is now observable, not silent.

Verified: 2 new tests in `TradePlanBuilder.test.ts` (rescue is requested and granted on a successful emission; confirmed via a live `TEMPORARY_DATA_RESCUE_GRANTED` event in the test log) — 32/32 tests green in that file; 105/105 across the five directly-affected suites (`TradePlanBuilder`, `SnapshotScanner`, `RthHandoff.integration`, `architecture.protection`, `MarketDataWorker`).

### 13.3 Two residual limitations — documented, not fixed in this pass

1. **No retry after a gate-13 rejection.** Even with the rescue fix, if the very first tick genuinely hasn't arrived by the moment ChiefTrader evaluates, that day's PRIMARY-tier idea for that symbol is still lost — `TradePlanBuilder` does not re-fire later in the day for a plan that already got one emission attempt. A more complete fix (re-emitting when a `REVALIDATING`/`READY` plan reaches real RTH-open liquidity) would be a legitimate design improvement, but it changes *when* trades can actually fire (potentially at RTH open, not only premarket) — a scope decision, not a bug fix, and is not made here without asking first.
2. **`ExtendedHoursLiquidityCache` (§11.5) is in-memory only, not persisted.** A process restart — which is exactly what §12.6 requires to pick up the new flags — clears it. The first extended-hours order attempt for every symbol after every restart will fail closed with `EXTENDED_HOURS_NO_LIQUIDITY_DATA` until the background refresh completes for that symbol. This is the same fail-closed discipline as the data-freshness gate, self-heals within one refresh cycle, and is a materially smaller-impact gap than 13.2 (affects only the first attempt per symbol per process lifetime, not every single day) — flagged here rather than fixed, since a persisted cache is a real design decision (which store, what TTL-on-restart semantics) rather than a bounded bug fix.

### 13.4 Real-market-conditions check — honestly not verifiable in this environment

Whether the extended-hours gates (spread/quote-age/ADV/notional) behave correctly under genuine live premarket order flow cannot be confirmed by unit or integration tests alone — those prove the gate logic is internally correct against constructed inputs, not that real Alpaca/IBKR premarket data will look the way the tests assume. No live premarket soak test was run as part of this pass. **Recommendation to the operator:** watch the first several real premarket cycles closely via the `PremarketIntelligence` tab's new ARMED badge and the `RiskGateHistoryPanel` (gate 25's real pass/fail detail will be visible there for any order that reaches it), rather than treating this pass's test-suite-green result as equivalent to live validation.

### 13.5 What was NOT re-litigated

The audit request's framing suggested treating this as a broader review of the whole discovery→execution path. This section deliberately stayed scoped to the three concrete questions raised (Java-quant conflict, price freshness, extended-hours gate correctness) rather than re-auditing already-covered ground (RiskEngine's 25 gates, ChiefTrader consensus math, OMS order construction) — none of which this pass touched and all of which remain exactly as documented in §5-§9 and the original Phase 5 work.
