...
const intelTabRegex = /\{activeTab === "intel" && \([\s\S]*?\n        \)\}\n\n/;
...
if (intelTabRegex.test(content)) {
  content = content.replace(intelTabRegex, '');
  console.log("Intel tab removed");
} else {
  console.log("Could not find intel tab, skipping removal");
}
