 ...
 * network calls are added by this function.
 */
export async function runRankingCycle(inputs: RankingInput[], now: Date = new Date()): Promise<RankedCandidate[]> {
  if (inputs.length === 0) return [];
  const symbols = inputs.map((i) => i.symbol);
  const cycleAt = now.toISOString();

  const [newsScores, agentScores, previousRanks] = await Promise.all([
  ...
  const ranked = rankCandidates(scored, previousRanks, PROMOTE_THRESHOLD, REJECT_THRESHOLD);
