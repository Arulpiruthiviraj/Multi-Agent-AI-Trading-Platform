...
export const STRATEGY_TYPICAL_HOLDING_PERIOD: Record<string, string> = {
  ...
  VWAP_VOLUME_STRUCTURE: 'Intraday to a few sessions - VWAP pullback in an established structure with RVOL. UNVALIDATED.',
  OPENING_RANGE_BREAKOUT: 'Same session as the opening range - invalidated if price re-enters the range. UNVALIDATED. Requires real intraday bars.',
  VWAP_MEAN_REVERSION: 'Very short-term - fade an extension vs session VWAP back toward VWAP in a range. UNVALIDATED.',
  DONCHIAN_CHANNEL_BREAKOUT: 'Short-term (hours to days) - prior N-bar channel break with trend strength. UNVALIDATED. Backtest is long-only.',
  MA_CROSSOVER: 'Medium-term - SMA50/SMA200 stack with EMA alignment. UNVALIDATED. Current MAs only, not a same-bar cross event.',
  OSCILLATOR_MOMENTUM: 'Short-term - RSI midline + MACD histogram + ROC. UNVALIDATED. Divergence is a feature, not this entry.',
  BOLLINGER_VOLATILITY: 'Short-term - Keltner expansion break using existing volatility features. UNVALIDATED.',
  PREVIOUS_PERIOD_BREAKOUT: 'Same session to a few days - previous-day high/low break with RVOL. UNVALIDATED.',
  CANDLESTICK_REVERSAL: 'Very short-term - detected 1-2 bar patterns at S/R. UNVALIDATED. Other named candles are aliases, not extra detectors.',
  GAP_CONTINUATION: 'Same session - UTC-day gap with RVOL and VWAP. UNVALIDATED. Not an earnings/news calendar strategy.',
