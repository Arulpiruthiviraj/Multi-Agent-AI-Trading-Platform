...
export const EXPLAINER_CATALOG = {
  allocatedCapital: {
    ...
    how: 'This is `settings.budget` (Allocated Budget Limit), saved from the gear on this card or Command Center via POST `/api/v1/config/settings`. RiskEngine gate `argus_capital_allocation` blocks BUYs that would exceed remaining allocation after open positions and pending buys. Autobot Start still rejects if allocation exceeds the active broker\'s buying power/cash.',
  },
  portfolioValuation: {
  ...
  totalEquity: {
    ...
    how: 'Hero ribbon reads GET /api/v1/portfolio `equity` (active broker cash + position market value). Distinct from Argus allocated capital. `--` means that call has not returned — not that paper cash is zero.',
  },
  cashBalance: {
    title: 'Cash Balance',
    ...
    how: 'Broker `cash` from GET /api/v1/portfolio. Internal paper default is `tradingSafety.internalPaperDefaultCash` ($100k). Buying power can differ (margin). `--` is a missing snapshot, not empty paper cash.',
  },
  positionsValuation: {
    ...
    how: 'Sum of broker position `marketValue` (else qty × price) from GET /api/v1/portfolio. Empty book is $0.00, not `--`. `--` only if the broker call has not returned.',
  },
  unrealizedPnl: {
    ...
    how: 'Sum of position `unrealizedPnl` from the broker, or broker `unrealizedPnl` when present. No open positions is $0.00. Realized P&L is a different field and stays DATA UNAVAILABLE until a filled SELL books `trades.profitLoss`.',
  },
  portfolioHealthscore: {
    ...
    how: 'No backend currently writes `health_score`. The ribbon shows `--` rather than fabricating 0–100. Live risk is the ordered RiskEngine gates, not this number.',
  },
  sqliteWal: {
  ...
  tabObservatory: {
    ...
    how: 'Assembles consensus, risk_assessments, gates, and fills from SQLite. Missing fields were never written. NO_CONSENSUS + Decision NO TRADE + Outcome N/A is a real closed cycle with no fill — hover those cells for why.',
  },
  tabActivity: {
    title: 'Activity Log',
    what: 'Chronological in-process log of EventBus decisions for this Node session (ideas, consensus, risk gates, fills).',
    why: 'Useful for debugging loops; not a substitute for event_traces.',
    how: 'TradingEngine.history from TRADE_IDEA_GENERATED / CHIEF_APPROVED_IDEA / DESK_NO_TRADE / RISK_ASSESSMENT_COMPLETED / ORDER_EXECUTED. Safe published fields only (agent, side, confidence, gate, traceId, price, NO_TRADE code). Durable lifecycle is Observatory.',
  },
  tabDiagnostics: {
    title: 'Diagnostics',
  ...
  tabSettings: {
    title: 'Settings & Keys',
    what: 'Budget, risk knobs, broker selection, encrypted API keys, and structural deployment readiness.',
    why: 'Allocation and keys are how the live path is allowed to talk to brokers and models. Integrity checks sit here next to the credentials they depend on.',
    how: 'settings row + encrypted brokerConnections. Deployment readiness uses GET /api/v1/system/integrity (structural only — LIVE remains NO-GO). Full check list also under Validation.',
  },
  tabDocumentation: {
    title: 'Documentation',
  ...
  engineRunBadge: {
    ...
    how: 'Derived from TradingEngine.state.enabled (same flag Observatory AutoBot RUNNING/STOPPED uses) and tradingState !== EMERGENCY_STOP. Kill switch is still RiskEngine emergency_stop.',
  },
  operationMode: {
  ...
  handsOffMode: {
    ...
    what: 'Opens Mission Control, where Autobot start/stop and allocated budget actually live.',
    why: 'The old Hands-Off toggle looked like a safety switch and did nothing.',
    how: 'Navigates to the command tab. Autobot on/off is TradingEngine.state.enabled. This control does not skip RiskEngine.',
  },
  realizedPerformance: {
