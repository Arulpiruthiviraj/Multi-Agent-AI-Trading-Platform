...
interface AgentComparisonModalProps {
  isOpen: boolean;
  onClose: () => void;
  mockAgentComparativeMetrics: Record<string, { sharpe: number, drawdown: number, wins: number }>;
  mockAgentRoiData: any[];
}

export default function AgentComparisonModal({
  isOpen,
  onClose,
  mockAgentComparativeMetrics,
  mockAgentRoiData
  ...
  const normalizedData = useMemo(() => {
    if (!mockAgentRoiData.length) return [];
    
    // Find initial values for normalization (start at 0)
    const initialAlpha = mockAgentRoiData[0][agentAlpha] || 0;
    const initialBeta = mockAgentRoiData[0][agentBeta] || 0;
    
    return mockAgentRoiData.map(point => ({
      date: point.date,
  ...
  }, [mockAgentRoiData, agentAlpha, agentBeta]);
                ...
                <table className="w-full text-xs text-left">
                  ...
                  <tbody className="divide-y divide-slate-800">
                    <tr className="hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-4 py-3 text-white font-mono">{mockAgentComparativeMetrics[agentAlpha]?.sharpe}</td>
                      <td className="px-4 py-3 text-white font-mono">{mockAgentComparativeMetrics[agentBeta]?.sharpe}</td>
                    </tr>
                    <tr className="hover:bg-slate-800/20 transition-colors">
                      <td className="px-4 py-3 text-slate-300">Max Drawdown</td>
                      <td className="px-4 py-3 text-rose-400 font-mono">{mockAgentComparativeMetrics[agentAlpha]?.drawdown}%</td>
                      <td className="px-4 py-3 text-rose-400 font-mono">{mockAgentComparativeMetrics[agentBeta]?.drawdown}%</td>
                    </tr>
                    <tr className="hover:bg-slate-800/20 transition-colors">
                      <td className="px-4 py-3 text-slate-300">Total Wins</td>
                      <td className="px-4 py-3 text-emerald-400 font-mono">{mockAgentComparativeMetrics[agentAlpha]?.wins}</td>
                      <td className="px-4 py-3 text-emerald-400 font-mono">{mockAgentComparativeMetrics[agentBeta]?.wins}</td>
                    </tr>
                    <tr className="hover:bg-slate-800/20 transition-colors">
                      <td className="px-4 py-3 text-slate-300">Win Rate</td>
                      <td className="px-4 py-3 text-slate-200 font-mono">{((mockAgentComparativeMetrics[agentAlpha]?.wins / 300) * 100).toFixed(1)}%</td>
                      <td className="px-4 py-3 text-slate-200 font-mono">{((mockAgentComparativeMetrics[agentBeta]?.wins / 300) * 100).toFixed(1)}%</td>
                    </tr>
                  </tbody>
                </table>
              </div>
