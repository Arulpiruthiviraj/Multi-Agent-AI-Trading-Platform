  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setData((prevData) => {
        ...
        const children = newData[0].children;
        
        // Randomly adjust weights slightly
        let total = 0;
        children.forEach((child: any) => {
          const change = (Math.random() * 4) - 2;
          child.size = Math.max(5, child.size + change);
          total += child.size;
        });
        
        // Normalize to 100%
        children.forEach((child: any) => {
          child.size = (child.size / total) * 100;
          child.value = Number(child.size.toFixed(1));
        });
        
        // Sort by size descending for better visualization
        newData[0].children.sort((a: any, b: any) => b.size - a.size);
