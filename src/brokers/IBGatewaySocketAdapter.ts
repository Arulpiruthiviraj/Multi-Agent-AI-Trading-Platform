...
export class IBGatewaySocketAdapter implements BrokerPlugin {
  ...
  /** 2026-09-23 spread forensic follow-up: real live BID(1)/ASK(2)/LAST(4) field-typed ticks,
   *  alongside setQuoteSink's live sink - see IbkrSocketSession.bidAskTickHandler's doc comment. */
  setBidAskTickHandler(handler: ((symbol: string, field: number, price: number) => void) | null): void {
    this.session.setBidAskTickHandler(handler);
  }
