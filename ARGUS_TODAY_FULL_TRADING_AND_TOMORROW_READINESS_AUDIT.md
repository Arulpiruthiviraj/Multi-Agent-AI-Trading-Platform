...
**Kronos pre-RTH (2026-08-21 ~07:44 ET) — EXPECTED, not a DEAD boot bug:** Spot-check at ~07:44 America/New_York (pre-RTH; open 09:30 ET) with Autobot OFF showed `KronosEngine` `currentState=IDLE`, `healthLabel=DEAD`, `lastTickAt=null` while Chronos `:8008/health` was `ok` and engine pid 26000 had `marketDataConnected=true`. `ArgusCoreBoot` starts `kronosForecastAgent` when the pipeline agent is enabled (`keepsBackgroundPipeline: true`); `enabled=true` / `available=true` on `/status` agents. Heartbeat only advances on `MARKET_DATA`, and `MarketDataWorker.maybeEmitMarketData` returns without emit unless `isAutobotTradingEnabled()` (Autobot on + `TRADING_ENABLED`) — unit-tested. With Autobot intentionally OFF, no ticks reach Kronos, so IDLE/`lastTickAt=null` → DEAD label is waiting-for-ticks, not a failed start. No code change; do not enable Autobot solely to green this lamp. Expect the same until Autobot ON (and meaningful flow around RTH).
