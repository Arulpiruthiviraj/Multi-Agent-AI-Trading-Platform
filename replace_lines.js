...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('id="intelligence-view"')) {
    startIdx = i;
  }
  // Keep looking for the end block of the intelligence view.
  if (startIdx !== -1 && lines[i].includes('{activeTab === "agents" && (() => {')) {
    endIdx = i - 2; // Typically ')}' is right before this line
    break;
  }
}
